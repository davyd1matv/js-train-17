...
function replaceText(word, replacement, text) {
  let w = new RegExp(word, "g");
  return text.replaceAll(w, replacement);

  //   let w = /(word)/g;

  //   return text.replace(w, replacement);
...
function checkWord(word, text) {
  const searchI = new RegExp(word, "i");
  return searchI.test(text);
...
function checkRegexMethods(regex) {
  ...
  if (regex.dotAll || regex.multiline || regex.sticky) {
    result.push("dotAll", "multiline", "sticky");
  }
  return result;
